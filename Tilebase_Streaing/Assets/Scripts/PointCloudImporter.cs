    ...
    public class PlyImporter : UnityEditor.AssetImporters.ScriptedImporter
    {
        ...
        static int GetPropertySize(DataProperty p)
        {
            switch (p)
            {
                ...
                //16ビットの座標
                case DataProperty.Int16X: return 2;
                case DataProperty.Int16Y: return 2;
                case DataProperty.Int16Z: return 2;
        ...
        public Mesh ImportAsMesh(byte[] data, int downloadIndex)
        {
            try
            {
                ...
                mesh.name = downloadIndex.ToString();
                ...
                if (col[0] == "property")
                {
                            ...
                            col[1] == "int16" || col[1] == "uint16")
                    {
                        switch (prop)
                        {
                            ...
                            // //16ビットの座標を読む
                            case DataProperty.SingleX: prop = DataProperty.Int16X; break;
                            case DataProperty.SingleY: prop = DataProperty.Int16Y; break;
                            case DataProperty.SingleZ: prop = DataProperty.Int16Z; break;
                        }
        ...
        DataBody ReadDataBody(DataHeader header, BinaryReader reader)
        {
            ...
            {
                double scaledValue = (double)value; // Scale the short value back to its original range
            ...
            for (var i = 0; i < header.vertexCount; i++)
            {
                foreach (var prop in header.properties)
                {
                    switch (prop)
                    {
                        ...
                        //16bitの座標を読む
                        case DataProperty.Int16X: x = ShortToFloat(reader.ReadInt16()); break;
                        case DataProperty.Int16Y: y = ShortToFloat(reader.ReadInt16()); break;
                        case DataProperty.Int16Z: z = ShortToFloat(reader.ReadInt16()); break;
